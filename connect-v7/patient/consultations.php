<?php
/**
 * patient/consultations.php - صفحة المحادثات
 * CONNECT+ - الإصدار 2.0
 */
...
// تحويل بيانات المحادثات إلى JSON للتشخيص
...
<body>
    <div class="content-wrapper">
        ...
        <div class="main-content">
            ...
            <div class="card-header">
                ...
                <button class="btn btn-primary" onclick="openSideModal('newMessageModal')">
            ...
            <div class="chat-container">
                ...
                <div class="conversations-list">
                    <button class="new-message-btn" onclick="openSideModal('newMessageModal')">
                        ...
                        <?php foreach ($conversations as $c): ?>
                            <div class="conversation-item <?= $c['unread'] ? 'unread' : '' ?>" onclick="loadConversation(<?= $c['other_id'] ?>)">
                                <div class="conversation-avatar">
                                    ...
                                    <?php else: ?>
                                        <i class="fas fa-<?= $c['role'] === 'doctor' ? 'user-md' : 'user' ?>"></i>
                                    <?php endif; ?>
                                </div>
    ...
    <!-- نافذة رسالة جديدة -->
    <div class="side-modal" id="newMessageModal">
        <div class="side-modal-header">
            <h3><i class="fas fa-pen"></i> رسالة جديدة</h3>
            <button class="close-side-modal" onclick="closeSideModal('newMessageModal')">&times;</button>
        </div>
            ...
            <div class="form-group">
                ...
                <select id="newMessageReceiver" class="form-control">
                    ...
                    <?php foreach ($doctors as $d): ?>
                        <option value="<?= $d['id'] ?>">د. <?= htmlspecialchars($d['full_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    function loadConversation(otherId) {
        console.log('تحميل المحادثة مع:', otherId);
        ...
        fetch('../api/get_user_info.php?id=' + otherId)
            .then(res => res.json())
            .then(data => {
                console.log('معلومات المستخدم:', data);
                if (data.success) {
                    ...
                    document.getElementById('chatAvatar').innerHTML = user.profile_image 
                        ...
                        : `<i class="fas fa-${user.role === 'doctor' ? 'user-md' : 'user'}"></i>`;
                    
                    document.getElementById('chatStatus').innerHTML = user.is_available 
                        ? '<span style="color: var(--success);"><i class="fas fa-circle"></i> متصل</span>' 
                        : '<span style="color: var(--gray);"><i class="fas fa-circle"></i> غير متصل</span>';
                }
            })
            .catch(error => {
    ...
    fetch('../api/get_messages.php?other_id=' + currentChatId + '&after=' + lastMessageId)
        ...
        .then(data => {
            if (data.success && data.messages && data.messages.length > 0) {
                // تحديث آخر ID للرسائل
                const newLastId = Math.max(...data.messages.map(m => m.id));
                
                // إذا كانت هذه أول مرة نجلب فيها الرسائل
                if (lastMessageId === 0) {
                    // عرض كل الرسائل
...
function appendMessages(messages) {
    ...
    messages.forEach(msg => {
        ...
        const existingMessages = area.querySelectorAll('.message');
        let exists = false;
        
        existingMessages.forEach(existing => {
            if (existing.getAttribute('data-message-id') == msg.id) {
                exists = true;
            }
        });
...
function sendMessage() {
    ...
    .then(data => {
        if (data.success) {
            // إضافة الرسالة الجديدة مباشرة (بدون انتظار التحديث)
            const area = document.getElementById('messagesArea');
            const messageDiv = document.createElement('div');
            ...
            // تحديث lastMessageId إذا كان لدينا ID
    ...
    function sendMessage() {
        ...
        console.log('إرسال رسالة إلى:', currentChatId, 'الرسالة:', msg);
    ...
    function sendNewMessage() {
        ...
        console.log('إرسال رسالة جديدة إلى:', receiver, 'الرسالة:', message);
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                closeSideModal('newMessageModal');
